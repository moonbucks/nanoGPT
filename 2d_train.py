...
print(f'L86 rank={_rank}, local_rank={_local_rank}')
...
model = GPT(twod_mesh, gptconf, device, pp_size)
#model = GPT(gptconf)
...
def tp(model, n_layer, mesh, offset=0, tp_dim=0):
  ...
  for i in range(n_layer):
    ...
    parallelize_module(block, mesh, {'attn.q': ColwiseParallel(),
                                     ...
                                     tp_mesh_dim=tp_dim) # why not work?
...
def pp_and_tp(model, n_layer, mesh):
  from pippy.compile import compile_stage
  from pippy.microbatch import TensorChunkSpec, sum_reducer

  pp_dim, tp_dim = 0, 1
  pp_rank, tp_rank = _local_rank // tp_size, _local_rank % tp_size
  pp_groups = mesh.get_dim_groups()[pp_dim]

  ## TP
  ## IMPORTANT set tp_size properly
  offset = 0
  for i in range(n_layer):
    block = model.get_submodule(f'transformer.h.{i+offset}')
    block.attn.tp_size = tp_size # which is mesh.mesh.size(0) 

  tp(model, n_layer, mesh, 0, tp_dim)

  #for i in range(n_layer):
  #  block = model.get_submodule(f'transformer.h.{i+offset}')
  #  block.attn.tp_size = 1 # to not bother compile_stage 

  print('L319 TP Success')

  # PP
  X, Y = get_batch('train')
  output_chunk_spec = (TensorChunkSpec(0), sum_reducer)

  # before compile_stage, tp_size of model needs to be set as 1
  stage = compile_stage(model, pp_rank, world_size, pp_size, device, pp_groups, 
            example_inputs=[X, Y],
            #output_chunk_spec=output_chunk_spec,
            )
  
  print('L332 PP Success')
  ...
  for name, module in stage.submod.named_modules():
    ...
    if name.endswith('attn_q') or name.endswith('attn_k') or name.endswith('attn_v'):
      plan[name] = ColwiseParallel()
    elif name.endswith('attn_c_proj'):
      plan[name] = RowwiseParallel()
    elif name.endswith('mlp_c_fc'):
      plan[name] = ColwiseParallel()
    elif name.endswith('mlp_c_proj'):
      plan[name] = RowwiseParallel()

  parallelize_module(stage.submod, mesh, plan, tp_mesh_dim=tp_dim)

  for name, module in stage.submod.named_modules():
    if "attn" in name:
      rank_print(f'name, {module.tp_size}, {module}')
  """
...
model, stage = pp_and_tp(model, n_layer, twod_mesh)
